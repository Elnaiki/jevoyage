...
export default function Auth() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup') {
        if (!fullName.trim()) { setError('Nom complet requis'); setLoading(false); return; }
        if (!phone.trim()) { setError('Téléphone requis'); setLoading(false); return; }
        if (!password.trim()) { setError('Mot de passe requis'); setLoading(false); return; }
      ...
      const identifier = mode === 'signup' ? phone : loginPhone;
      ...
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 mt-1">
            {mode === 'login' ? 'Connectez-vous pour continuer' : 'Créez votre compte JeVoyage'}
          </p>
        </div>
